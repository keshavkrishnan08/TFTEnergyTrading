#!/usr/bin/env python3
"""
Train VSN models with optimized trade selection.

Key improvements:
1. LSTM: Higher probability thresholds to reduce over-trading
2. TCN: Fixed architecture and better hyperparameters
3. Configurable trade thresholds to match TFT's selectivity

Usage:
    python train_vsn_optimized.py lstm --threshold 0.58
    python train_vsn_optimized.py tcn --threshold 0.55
"""
import torch
...
from src.models.trading_models import ProbabilityCalibrator, TradingDecisionEngine
...
class SelectiveBacktest(AdvancedBacktest):
    """
    Backtest with configurable probability threshold for trade filtering.
    """
    ...
    def run_backtest(self, predictions, labels, dates, dataset, price_data=None,
        ...
        # Temporarily modify the decision engine's threshold
        original_threshold = 0.52

        # Monkey-patch the make_decision method to use our threshold
...
def train_model_with_threshold(model_type='lstm', threshold=0.58):
    """
    Train VSN model with optimized settings and custom trade threshold.

    Args:
        model_type: 'lstm' or 'tcn'
        threshold: Probability threshold for taking trades (0.52-0.65)
    """
    print("="*80)
    print(f"{model_type.upper()} WITH VSN - OPTIMIZED (Threshold={threshold})")
    print("="*80)
    ...
    for year in test_years:
        ...
        if model_type == 'lstm':
            # LSTM: Proven architecture, just needs better trade filtering
            config.LSTM_HIDDEN_SIZE = 128
            config.LSTM_LAYERS = 2
            config.DROPOUT = 0.3
            model = LSTMWithVSN(config).to(device)
        elif model_type == 'tcn':
            # TCN: More aggressive regularization to prevent overfitting
            config.LSTM_HIDDEN_SIZE = 96  # Reduce capacity
            config.DROPOUT = 0.4  # Increase dropout
    ...
    # Use selective backtest with custom threshold
    ...
    # Save
    output_dir = Path(f'experiments/{model_type}_vsn_opt_t{int(threshold*100)}')
    ...
    # Print results
    print("\n" + "="*80)
    print(f"{model_type.upper()} WITH VSN - RESULTS (Threshold={threshold})")
    print("="*80)
...
if __name__ == '__main__':
    ...
    parser.add_argument('model', choices=['lstm', 'tcn'], help='Model type')
    parser.add_argument('--threshold', type=float, default=0.58,
                       help='Probability threshold for taking trades (default: 0.58)')
    args = parser.parse_args()

    train_model_with_threshold(model_type=args.model, threshold=args.threshold)
