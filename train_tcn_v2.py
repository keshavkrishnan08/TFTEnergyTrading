#!/usr/bin/env python3
"""
Train improved TCN V2 with VSN.

Usage:
    python train_tcn_v2.py --threshold 0.56
"""
import torch
...
from src.models.tcn_with_vsn_v2 import ImprovedTCNWithVSN
...
# Import SelectiveBacktest from the other script
class SelectiveBacktest(AdvancedBacktest):
    """Backtest with configurable probability threshold."""
...
def train_tcn_v2(threshold=0.56):
    """
    Train improved TCN V2 with optimized settings.
    """
    print("="*80)
    print(f"TCN V2 WITH VSN - OPTIMIZED (Threshold={threshold})")
    print("="*80)
    ...
    for year in test_years:
        ...
        # Initialize improved TCN V2
        print(f"\n  Initializing TCN V2 with VSN...")
        config.LSTM_HIDDEN_SIZE = 96  # Reduced capacity
        config.DROPOUT = 0.4  # Increased dropout
        model = ImprovedTCNWithVSN(config).to(device)
        ...
        # Strong regularization for TCN
        optimizer = optim.AdamW(
            model.parameters(),
            lr=8e-4,  # Slightly lower LR
            weight_decay=3e-4,  # Higher weight decay
            betas=(0.9, 0.999)
        )
    ...
    # Save
    output_dir = Path(f'experiments/tcn_v2_vsn_opt_t{int(threshold*100)}')
    ...
    # Print results
    print("\n" + "="*80)
    print(f"TCN V2 WITH VSN - RESULTS (Threshold={threshold})")
    print("="*80)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threshold', type=float, default=0.56,
                       help='Probability threshold for taking trades (default: 0.56)')
    args = parser.parse_args()

    train_tcn_v2(threshold=args.threshold)
