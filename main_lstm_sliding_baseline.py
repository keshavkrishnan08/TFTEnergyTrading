#!/usr/bin/env python3
"""
LSTM V7 Baseline with Same Execution Model as TFT V8 Sliding
...
def main():
    print("="*80)
    print("LSTM V7 BASELINE - SLIDING WINDOW (2018-2022)")
    print("="*80)
    ...
    for year in test_years:
        ...
        # Create raw_prices dict for backtest
        raw_prices = {
        ...
        print(f"\n  Initializing LSTM V7 model...")
        config.LSTM_HIDDEN_SIZE = 128
        config.LSTM_LAYERS = 2
        config.DROPOUT = 0.5

        model = SimpleLSTM(config).to(device)
        ...
        # Only add valid dates (after sequence offset)
    ...
    # Save results
    output_dir = Path('experiments/lstm_v7_sliding_baseline')
    output_dir.mkdir(parents=True, exist_ok=True)

    # Save metrics
    metrics_df = pd.DataFrame([{k: v for k, v in results.items() if k not in ['equity_curve', 'daily_returns', 'trades', 'trades_df', 'asset_stats', 'runners', 'dates']}])
    ...
    # Print summary
    print("\n" + "="*80)
    print("LSTM V7 BASELINE - RESULTS")
    print("="*80)
