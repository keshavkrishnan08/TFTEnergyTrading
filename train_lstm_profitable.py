#!/usr/bin/env python3
"""
Train LSTM with the profitable configuration found from grid search.

Best Config (from grid search on 2022):
- Threshold: 0.45
- Loss: Focal Loss (alpha=0.25, gamma=2.0)
- Return: +1.19% on 2022
- Trades: 72, Win Rate: 41.7%

This script trains on full dataset (2018-2022) with this optimized config.
"""
import torch
...
def train_lstm_profitable():
    """Train LSTM with optimized profitable configuration."""
    print("="*80)
    print("LSTM WITH FOCAL LOSS (Threshold=0.45) - PROFITABLE CONFIG")
    print("="*80)
    ...
    # Backtest
    print(f"\n{'='*80}")
    print(f"Running Backtest (Threshold=0.45, Focal Loss)...")
    print(f"{'='*80}\n")
    ...
    backtest = SelectiveBacktest(config, probability_threshold=0.45)
    ...
    # Save
    output_dir = Path('experiments/lstm_focal_t45')
    ...
    # Print results
    print("\n" + "="*80)
    print(f"LSTM WITH FOCAL LOSS - FINAL RESULTS (Threshold=0.45)")
    print("="*80)
