# train_tft_v8.py
"""
V8 TFT Training Script with Strict Temporal Split.
- Trains Temporal Fusion Transformer
- 70/15/15 Split (Train/Calib/Hidden Test)
- Isotonic Calibration on Calib set
- NO DATA LEAKAGE
"""
import torch
...
def train():
    print("="*80)
    print("TFT V8 TRAINING")
    print("="*80)
    
    config = Config()
    # Add TFT-specific config
    ...
    # 3. Strict Temporal Split (70/15/15)
    total_len = len(df)
    ...
    print(f"  Hidden Test: {len(test_df)} samples (up to {test_df.iloc[-1]['Date']})")
