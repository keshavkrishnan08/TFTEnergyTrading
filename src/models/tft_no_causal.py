# src/models/tft_no_causal.py
"""
ABLATION TEST: Temporal Fusion Transformer WITHOUT Causal Attention Masking
- Removes causal mask to allow bidirectional attention
- This creates lookahead bias but tests if causal constraints hurt performance
- Keeps all other components: VSN, GRN, position encoding
...
class VariableSelectionNetwork(nn.Module):
    """
    Optimized Variable Selection Network (VSN) for CPU.
    Instead of 200+ independent GRNs, we use a shared linear layer
    for feature embedding and a single GRN for weighting.
    """
...
class BidirectionalMultiHeadAttention(nn.Module):
    """
    ABLATION: Multi-Head Attention WITHOUT Causal Mask.
    Allows attention to ALL positions (including future).
    This creates lookahead bias in training but tests if causal constraints matter.
    """
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # ❌ NO CAUSAL MASK: Allow full bidirectional attention
        # (In original TFT, this would mask future positions)
...
class TFT_NoCausal(nn.Module):
    """
    ABLATION: Temporal Fusion Transformer WITHOUT Causal Attention Masking

    Architecture:
    1. ✅ KEPT: Variable Selection Network (learns feature importance)
    2. ✅ KEPT: Positional Encoding (temporal awareness)
    3. ❌ REMOVED: Causal masking in attention
    4. ✅ KEPT: Multi-Head Self-Attention (now bidirectional)
    5. ✅ KEPT: Gated Residual Networks (information gating)
    6. ✅ KEPT: Multi-Asset Output Heads

    This tests whether causal constraints are necessary or if bidirectional
    attention can capture patterns better (at the cost of lookahead bias).

    WARNING: This creates lookahead bias in backtest. Use only for ablation study.
    """
    def __init__(self, config):
        ...
        # ❌ CHANGED: Transformer encoder layers with BIDIRECTIONAL attention
        self.attention_layers = nn.ModuleList([
            BidirectionalMultiHeadAttention(self.hidden_dim, self.num_heads, self.dropout)
    ...
    def forward(self, x, time_features):
        ...
        for i in range(self.num_layers):
            # Multi-head attention WITHOUT causal mask
            attn_out, attn_weights = self.attention_layers[i](
                hidden, hidden, hidden, mask=None  # No mask!
            )
...
if __name__ == "__main__":
    ...
    model = TFT_NoCausal(config)

    # Count parameters
    no_causal_params = sum(p.numel() for p in model.parameters())
    print(f"TFT (No Causal) Parameters: {no_causal_params:,}")
    ...
    print("✓ TFT (No Causal) Ablation Model Test Passed!")
