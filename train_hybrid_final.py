#!/usr/bin/env python3
"""
Train Transformer-LSTM Hybrid to replace failed TCN.

This hybrid architecture should significantly outperform TCN by combining:
- Transformer attention mechanisms
- LSTM temporal modeling
- Variable Selection Network

Will use the best threshold found from LSTM grid search.
"""
import torch
...
from src.models.transformer_lstm_hybrid import TransformerLSTMHybrid
...
def train_hybrid(threshold=0.45):
    """Train Transformer-LSTM hybrid."""
    print("="*80)
    print(f"TRANSFORMER-LSTM HYBRID (Threshold={threshold})")
    print("="*80)
    ...
    for year in test_years:
        ...
        # Initialize Transformer-LSTM hybrid
        print(f"\n  Initializing Transformer-LSTM Hybrid...")
        config.LSTM_HIDDEN_SIZE = 128
        config.LSTM_LAYERS = 2
        config.DROPOUT = 0.3
        model = TransformerLSTMHybrid(config).to(device)
    ...
    # Save
    output_dir = Path(f'experiments/hybrid_t{int(threshold*100)}')
    ...
    # Print results
    print("\n" + "="*80)
    print(f"TRANSFORMER-LSTM HYBRID - RESULTS (Threshold={threshold})")
    print("="*80)
...
if __name__ == '__main__':
    ...
    train_hybrid(threshold=args.threshold)
