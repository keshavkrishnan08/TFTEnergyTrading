# main_tft_ablation_no_causal.py
"""
ABLATION TEST: TFT WITHOUT Causal Attention Masking
- Tests contribution of causal masking by removing it
- Uses bidirectional attention (allows seeing future in training sequences)
- Keeps same training pipeline, sliding windows, and execution model as V8
- Saves results to: experiments/tft_ablation_no_causal/

WARNING: This creates lookahead bias in training. Use only for ablation study.
The backtest results will be artificially inflated due to future information leakage.
"""
import torch
...
from src.models.tft_no_causal import TFT_NoCausal  # ABLATION: Import no-causal model
...
def run_ablation_no_causal():
    print("="*80)
    print("ABLATION TEST: TFT WITHOUT Causal Attention Masking")
    print("WARNING: This creates lookahead bias for demonstration purposes")
    print("="*80)
    ...
    EXPERIMENT_DIR = Path('experiments/tft_ablation_no_causal')
    ...
    for year in test_years:
        ...
        train_df = df[df['Date'].str.contains('|'.join([str(y) for y in range(year-5, year)]))] # 5 years training
        ...
        # ABLATION: Use TFT_NoCausal instead of TemporalFusionTransformer
        model = TFT_NoCausal(config).to(device)
    ...
    # Run Backtest
    print("\nRunning Advanced Backtest (No Causal)...")
    ...
    # Display & Save
    print("\n" + "="*80)
    print("ABLATION: TFT WITHOUT CAUSAL MASKING - RESULTS")
    print("="*80)
    ...
    print("="*80)
    print("\nComparison to V8 (Full TFT with Causal Masking):")
    ...
    print("="*80)
    print("\n⚠️  WARNING: Results include lookahead bias from bidirectional attention")
    print("If performance is BETTER, it's due to future information leakage (invalid)")
    print("If performance is WORSE, causal masking provides proper regularization")
    print("="*80)
    ...
    with open(EXPERIMENT_DIR / 'ABLATION_SUMMARY.txt', 'w') as f:
        f.write("="*80 + "\n")
        f.write("ABLATION TEST: TFT WITHOUT Causal Attention Masking\n")
        f.write("="*80 + "\n\n")
        f.write("HYPOTHESIS: Causal masking prevents lookahead bias and provides regularization\n\n")
        f.write("CHANGES FROM V8:\n")
        f.write("  - ❌ REMOVED: Causal attention masking (lower-triangular mask)\n")
        f.write("  - ✅ REPLACED: Bidirectional attention (can see all positions)\n")
        f.write("  - ✅ KEPT: VSN, GRN, position sizing, sliding windows\n\n")
        f.write("⚠️  CREATES LOOKAHEAD BIAS: Model can see future in training sequences\n\n")
        f.write("RESULTS:\n")
        f.write(f"  Total Return:    {results['total_return_pct']:+.2f}%\n")
        f.write(f"  Sharpe Ratio:    {results['sharpe_ratio']:.2f}\n")
        f.write(f"  Win Rate:        {results['win_rate']:.1%}\n")
        f.write(f"  Total Trades:    {results['total_trades']}\n\n")
        f.write("COMPARISON TO V8 (Full TFT with Causal Masking):\n")
        ...
        if results['total_return_pct'] > 245.23:
            f.write(f"  Removing causal mask improved performance by +{results['total_return_pct'] - 245.23:.2f}%\n")
            f.write("  ⚠️  This is due to LOOKAHEAD BIAS (future information leakage)\n")
            f.write("  ✓ Confirms causal masking is necessary to prevent overfitting\n")
            f.write(f"  ✓ Causal masking sacrifices ~{results['total_return_pct'] - 245.23:.2f}% for valid results\n")
        else:
            f.write(f"  Removing causal mask reduced performance by {245.23 - results['total_return_pct']:.2f}%\n")
            f.write("  Unexpected: bidirectional should have lookahead advantage\n")
            f.write("  Possible explanation: causal structure helps learning\n")

if __name__ == "__main__":
    run_ablation_no_causal()
